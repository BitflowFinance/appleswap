...
(define-fungible-token green-apples)
...
(define-read-only (get-balance (account principal))
  (ok (ft-get-balance green-apples account))
)

(define-read-only (get-balance-simple (account principal))
  (ft-get-balance green-apples account)
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (match (ft-transfer? green-apples amount sender recipient)
      response (begin
        (print memo)
        (ok response)
      )
...
(define-public (mint (amount uint) (recipient principal))
  (begin
    ...
    (ft-mint? green-apples amount recipient)
  )
)
...
(define-public (burn (burner principal) (amount uint))
  (begin
    ...
    (ft-burn? green-apples amount burner)
  )
)